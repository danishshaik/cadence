...
// Joint positions match design.pen centers (x + 10, y + 10)
...
export function LocationStep() {
  ...
  const handleJointPress = (id: JointLocationId) => {
    ...
    const current = formData.affectedJoints;

    // If bilateral is on, toggle both sides
    if (formData.bilateralSymmetry) {
      ...
      if (joint && "side" in joint) {
        ...
        if (bothSelected) {
          updateFormData({
            affectedJoints: current.filter((j) => j !== leftId && j !== rightId),
          });
        } else {
          const newJoints = current.filter((j) => j !== leftId && j !== rightId);
          updateFormData({ affectedJoints: [...newJoints, leftId, rightId] });
        }
        return;
      }
    }
    ...
    const updated = current.includes(id)
      ...
      : [...current, id];
    updateFormData({ affectedJoints: updated });
  };

  const toggleBilateral = () => {
    Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
    updateFormData({ bilateralSymmetry: !formData.bilateralSymmetry });
  };
    ...
    <View style={styles.container}>
        ...
        <View style={[styles.skeletonCard, { width: mapWidth, height: mapHeight }]}>
          ...
          {Object.entries(JOINT_POSITIONS).map(([id, pos]) => (
            <JointDot
              ...
              x={pos.x * mapWidth}
              y={pos.y * mapHeight}
              selected={formData.affectedJoints.includes(id as JointLocationId)}
      ...
      <View style={styles.footer}>
        <Pressable
          ...
          style={[styles.bilateralPill, formData.bilateralSymmetry && styles.bilateralPillActive]}
        >
          <Text
            style={[
              styles.bilateralText,
              formData.bilateralSymmetry && styles.bilateralTextActive,
            ]}
          >
            Both Sides: {formData.bilateralSymmetry ? "On" : "Off"}
          </Text>
        </Pressable>
